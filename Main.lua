...
local function GetOptionsCategoryID()
    -- 1) direkt versuchen
    local id = EasyDestroy.UI.Options and EasyDestroy.UI.Options.categoryID

    -- 2) notfalls initialisieren
    if not id and EasyDestroy.UI.Options and EasyDestroy.UI.Options.__init then
        EasyDestroy.UI.Options.__init()
        id = EasyDestroy.UI.Options.categoryID
    end

    -- 3) Fallback: über Namen auflösen (falls du optionsFrame.name kennst)
    if not id and EasyDestroy and EasyDestroy.UI and EasyDestroy.UI.Options
       and EasyDestroy.UI.Options.category and EasyDestroy.UI.Options.category.name then
        local cat = Settings.GetCategory(EasyDestroy.UI.Options.category.name)
        if cat then id = cat.ID end
    end
			...
			local ldb = LibStub:GetLibrary("LibDataBroker-1.1"):NewDataObject("EasyDestroy", {
				...
				OnClick = function(self, button)
					if button == "RightButton" then
						local id = GetOptionsCategoryID()
						if id then
							Settings.OpenToCategory(id)
						else
							print("|cffff5555EasyDestroy: Options-Kategorie nicht verfügbar.|r")
						end
