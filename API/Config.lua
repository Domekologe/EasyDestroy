...
function EasyDestroy.Config.ItemTypeFilterByFlags(flags)
    local list = {}

    if doDE then
        table.insert(list, { itype = 4 }) -- Armor
        table.insert(list, { itype = 2 }) -- Weapon
    end
    if doMill then
        table.insert(list, { itype = 7, stype = 9 }) -- Tradegoods: Herbs
    end
    if doPros then
        table.insert(list, { itype = 7, stype = 7 }) -- Tradegoods: Metal & Stone
    end
    return list
end
