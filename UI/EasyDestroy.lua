...
function EasyDestroyFrame.__init()
    ...
    EasyDestroyButton:SetScript("PostClick", function(self)
		...
		-- Force an inventory refresh shortly after the macro runs
		if C_Timer and C_Timer.After then
			C_Timer.After(0.30, function()
				-- This will run Inventory.UpdateInventory(), which then fires ED_INVENTORY_UPDATED_DELAYED
				EasyDestroy.Events:Call("ED_INVENTORY_UPDATED")
			end)
		else
			-- Fallback if timers aren’t available for some reason
			EasyDestroy.Events:Call("ED_INVENTORY_UPDATED")
		end
	end)
